...
export default function VerifyOtp() {
  ...
  const resend = async () => { try { await authService.resendOtp({ email }); toast.success("OTP resent"); } catch { toast.error("Failed to resend"); } };
  ...
  return (
    <AuthLayout title="Verify your email" subtitle={`We sent a 6-digit code to ${email || "your email"}`}>
      <div className="space-y-5">
        ...
        <p className="text-center text-sm text-muted-foreground">Didn't get it? <button onClick={resend} className="text-primary font-medium hover:underline">Resend OTP</button></p>
      </div>
    </AuthLayout>
  );
}
